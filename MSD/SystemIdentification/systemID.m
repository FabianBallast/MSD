...
omega_control = 100 * 2 * pi;
C = ((s^2 + 2 * zeta * omega * s + omega^2)/(1/s * k)) * (1/(s/omega_control + 1));
...
wd = 1/3 * wc;
wt = 3 * wc;
C_fb = kp*(1 + wi/s)*((1 + s/wd)/(1 + s/wt))*(wc/(s + wc));

[mag,phase,wout] = bode(G,{10^-2 * 2 * pi, 10^2 * 2 * pi});
mag = 20 * log10(squeeze(mag));
[magc,phasec,woutc] = bode(C,{10^-2 * 2 * pi, 10^2 * 2 * pi});
